...
void rptDisp::RPT_read(String^ fullfilename)
{
    ...
    tmpStr=readLineStr->Substring(0, 2);
    ...
    tmpStr=readLineStr->Substring(2, 2);
    this->RPT_date+=", "+tmpStr+"/20";
    tmpStr=readLineStr->Substring(4, 2);
...
String^ rptDisp::parse_M1(String^ InStr)
{
    ...
    array<String^>^result;
    result=InStr->Split(sep, StringSplitOptions::RemoveEmptyEntries);
    if(result[0]=="1"){   //it should not happen!! (circ aloway on with it)
        if(result[2]=="E")
